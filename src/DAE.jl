  module DAE
    ...
    @UniontypeDecl ClassPrefix
        ...
        const auxNamePrefix = "AUX"::String
         ...
         @Uniontype ElementSource begin
              @Record SOURCE begin
                      ...
                      instance #= the instance(s) this element is part of =#::ComponentPrefix
        ...
        const emptyElementSource = SOURCE(AbsynUtil.dummyInfo, nil, NOCOMPPRE(), nil, nil, nil, nil)::ElementSource
         ...
         @Uniontype SymbolicOperation begin
              ...
              @Record LINEAR_SOLVED begin
                      ...
                      jac::List{List{AbstractFloat}}
                      rhs::List{AbstractFloat}
                      result::List{AbstractFloat}
              end
         ...
         @Uniontype Element begin
              @Record VAR begin
                      ...
                      ty #= Full type information required =#::M_Type
              ...
              @Record FOR_EQUATION begin
                      ...
                      type_ #= this is the type of the iterator =#::M_Type
                      iterIsArray #= True if the iterator has an array type, otherwise false. =#::Bool
                      iter #= the iterator variable =#::Ident
                      index #= the index of the iterator variable, to make it unique; used by the new inst =#::Integer
        ...
        const T_ASSERTIONLEVEL = T_ENUMERATION(NONE(), Absyn.FULLYQUALIFIED(Absyn.IDENT("AssertionLevel")), list("error", "warning"), nil, nil)::M_Type
        ...
        const ASSERTIONLEVEL_WARNING = ENUM_LITERAL(Absyn.QUALIFIED("AssertionLevel", Absyn.IDENT("warning")), 2)::Exp
         ...
         @Uniontype Function begin
              @Record FUNCTION begin
                      ...
                      unusedInputs #= The indices of any inputs not used in the function. =#::List{Integer}
                      source #= the origin of the component/equation/algorithm =#::ElementSource
                      comment::Option{SCode.Comment}
              end
         ...
         @Uniontype FunctionDefinition begin
              ...
              @Record FUNCTION_DER_MAPPER begin
                      ...
                      conditionRefs::List{Tuple{Integer, derivativeCond}}
        ...
        module AvlTreePathFunction
          ...
          using ExportAll
              ...
              using #= Modelica extend clause =# BaseAvlTree
              ...
              Value = Option
                ...
                @ExtendedFunction addConflictDefault addConflictReplace()
         ...
         @Uniontype Var begin
              @Record TYPES_VAR begin
                      ...
                      bind_from_outside #= true if the binding has come from out of scope. This happens for derived record classes.
                                                         ...
                                                         will have to be supplied from outside for a default constructor of the owner record type =#::Bool
        ...
        EqualityConstraint = Option  #= contains the path to the equalityConstraint function,
         ...
         =#
        const T_REAL_DEFAULT = T_REAL(nil)::M_Type
        const T_INTEGER_DEFAULT = T_INTEGER(nil)::M_Type
        const T_STRING_DEFAULT = T_STRING(nil)::M_Type
        const T_BOOL_DEFAULT = T_BOOL(nil)::M_Type
        const T_CLOCK_DEFAULT = T_CLOCK(nil)::M_Type
        const T_ENUMERATION_DEFAULT = T_ENUMERATION(NONE(), Absyn.IDENT(""), nil, nil, nil)::M_Type
        const T_REAL_BOXED = T_METABOXED(T_REAL_DEFAULT)::M_Type
        const T_INTEGER_BOXED = T_METABOXED(T_INTEGER_DEFAULT)::M_Type
        const T_STRING_BOXED = T_METABOXED(T_STRING_DEFAULT)::M_Type
        const T_BOOL_BOXED = T_METABOXED(T_BOOL_DEFAULT)::M_Type
        const T_METABOXED_DEFAULT = T_METABOXED(T_UNKNOWN_DEFAULT)::M_Type
        const T_METALIST_DEFAULT = T_METALIST(T_UNKNOWN_DEFAULT)::M_Type
        const T_NONE_DEFAULT = T_METAOPTION(T_UNKNOWN_DEFAULT)::M_Type
        const T_ANYTYPE_DEFAULT = T_ANYTYPE(NONE())::M_Type
        const T_UNKNOWN_DEFAULT = T_UNKNOWN()::M_Type
        const T_NORETCALL_DEFAULT = T_NORETCALL()::M_Type
        const T_METATYPE_DEFAULT = T_METATYPE(T_UNKNOWN_DEFAULT)::M_Type
        const T_COMPLEX_DEFAULT = T_COMPLEX(ClassInf.UNKNOWN(Absyn.IDENT("")), nil, NONE()) #= default complex with unknown CiState =#::M_Type
        const T_COMPLEX_DEFAULT_RECORD = T_COMPLEX(ClassInf.RECORD(Absyn.IDENT("")), nil, NONE()) #= default complex with record CiState =#::M_Type
        const T_SOURCEINFO_DEFAULT_METARECORD = T_METARECORD(Absyn.QUALIFIED("SourceInfo", Absyn.IDENT("SOURCEINFO")), Absyn.IDENT("SourceInfo"), nil, 1, list(TYPES_VAR("fileName", dummyAttrVar, T_STRING_DEFAULT, UNBOUND(), false, NONE()), TYPES_VAR("isReadOnly", dummyAttrVar, T_BOOL_DEFAULT, UNBOUND(), false, NONE()), TYPES_VAR("lineNumberStart", dummyAttrVar, T_INTEGER_DEFAULT, UNBOUND(), false, NONE()), TYPES_VAR("columnNumberStart", dummyAttrVar, T_INTEGER_DEFAULT, UNBOUND(), false, NONE()), TYPES_VAR("lineNumberEnd", dummyAttrVar, T_INTEGER_DEFAULT, UNBOUND(), false, NONE()), TYPES_VAR("columnNumberEnd", dummyAttrVar, T_INTEGER_DEFAULT, UNBOUND(), false, NONE()), TYPES_VAR("lastModification", dummyAttrVar, T_REAL_DEFAULT, UNBOUND(), false, NONE())), true)::M_Type
        const T_SOURCEINFO_DEFAULT = T_METAUNIONTYPE(list(Absyn.QUALIFIED("SourceInfo", Absyn.IDENT("SOURCEINFO"))), nil, true, EVAL_SINGLETON_KNOWN_TYPE(T_SOURCEINFO_DEFAULT_METARECORD), Absyn.IDENT("SourceInfo"))::M_Type
         #=  Arrays of unknown dimension, eg. Real[:]
         =#

        const T_ARRAY_REAL_NODIM = T_ARRAY(T_REAL_DEFAULT, list(DIM_UNKNOWN()))::M_Type

        const T_ARRAY_INT_NODIM = T_ARRAY(T_INTEGER_DEFAULT, list(DIM_UNKNOWN()))::M_Type

        const T_ARRAY_BOOL_NODIM = T_ARRAY(T_BOOL_DEFAULT, list(DIM_UNKNOWN()))::M_Type

        const T_ARRAY_STRING_NODIM = T_ARRAY(T_STRING_DEFAULT, list(DIM_UNKNOWN()))::M_Type
         ...
         @Uniontype Type begin
              ...
              @Record T_ENUMERATION begin
                      ...
                      index #= the enumeration value index, SOME for element, NONE() for type =#::Option{Integer}
              ...
              @Record T_SUBTYPE_BASIC begin
                      ...
                      complexType #= complexType; A complex type can be a subtype of another (primitive) type (through extends) =#::M_Type
                      equalityConstraint::EqualityConstraint
              end
              ...
              @Record T_FUNCTION begin
                      ...
                      funcResultType #= Only single-result =#::M_Type
              ...
              @Record T_FUNCTION_REFERENCE_VAR begin
                      ...
                      functionType #= the type of the function =#::M_Type
              end

              @Record T_FUNCTION_REFERENCE_FUNC begin

                      builtin::Bool
                      functionType #= type of the non-boxptr function =#::M_Type
              end

              @Record T_TUPLE begin

                      types #= For functions returning multiple values. =#::List{M_Type}
        ...
        Dimensions = List  #= a list of dimensions =#
         ...
         @Uniontype Dimension begin
              ...
              @Record DIM_ENUM begin
                      ...
                      size #= The size of the enumeration. =#::Integer
              end
         ...
         @Uniontype Exp begin
              ...
              @Record RCONST begin
                      ...
                      real #= Real constants =#::AbstractFloat
              end
              ...
              @Record RANGE begin
                      ...
                      ty #= the (array) type of the expression =#::M_Type
              ...
              @Record CAST begin
                      ...
                      ty #= This is the full type of this expression, i.e. ET_ARRAY(...) for arrays and matrices =#::M_Type
                      exp::Exp
              end
              ...
              @Record EMPTY begin
                      ...
                      ty #= the type of the variable =#::M_Type
                      tyStr::String
              end
              ...
              @Record SHARED_LITERAL begin
                      ...
                      index #= A unique indexing that can be used to point to a single shared literal in generated code =#::Integer
        ...
        const callAttrOther = CALL_ATTR(T_UNKNOWN_DEFAULT, false, false, false, false, NO_INLINE(), NO_TAIL())::CallAttributes
         ...
         @Uniontype CallAttributes begin
              @Record CALL_ATTR begin
                      ...
                      ty #= The type of the return value, if several return values this is undefined =#::M_Type
                      tuple_ #= tuple =#::Bool
        ...
        ReductionIterators = List  #= NOTE: OMC only handles one iterator for now =#
         ...
         @Uniontype MatchCase begin
              @Record CASE begin
                      ...
                      jump #= the number of iterations we should skip if we succeed with pattern-matching, but don't succeed =#::Integer
                      info::SourceInfo
              end
         end
         ...
         @Uniontype MatchType begin
              ...
              @Record MATCH begin
                      ...
                      switch #= The index of the pattern to switch over, its type and the value to divide string hashes with =#::Option{Tuple{Integer, M_Type, Integer}}
              end
         end
         ...
         @Uniontype Pattern begin
              ...
              @Record PAT_CONSTANT begin
                      ...
                      ty #= so we can unbox if needed =#::Option{M_Type}
                      exp::Exp
              end
              ...
              @Record PAT_CALL_NAMED begin
                      ...
                      name::Absyn.Path
                      patterns::List{Tuple{Pattern, String, M_Type}}
              end
         ...
         @Uniontype ComponentRef begin
              @Record CREF_QUAL begin
                      ...
                      ident::Ident
                      identType #= type of the identifier, without considering the subscripts =#::M_Type
        ...
        module Connect
          ...
          @UniontypeDecl Set
              ...
              const NEW_SET = -1 #= The index used for new sets which have not
                yet been assigned a set index. =#::Integer
               ...
               @Uniontype ConnectorElement begin
                    @Record CONNECTOR_ELEMENT begin
                            ...
                            set #= Which set this element belongs to. =#::Integer
                    end
               end
               ...
               @Uniontype SetTrieNode begin
                    ...
                    @Record SET_TRIE_LEAF begin
                            ...
                            connectCount #= How many times this connector has been connected. =#::Integer
                    end
               end
              ...
              SetConnection = Tuple  #= A connection between two sets. =#
               ...
               @Uniontype OuterConnect begin
                    @Record OUTERCONNECT begin
                            ...
                            scope #= the scope where this connect was created =#::Prefix
               ...
               @Uniontype Sets begin
                    @Record SETS begin
                            ...
                            sets::SetTrie
                            setCount #= How many sets the trie contains. =#::Integer
