...
function promote(e::Expression, ty::M_Type, n::Integer) ::Tuple{Expression, M_Type}
  ...
  =#
  @assign dims = list(P_Dimension.Dimension.fromInteger(1) for i in dimensionCount(ty):n - 1)
  if ! listEmpty(dims)
...
function mapFoldShallow(exp::Expression, func::MapFunc, arg::ArgT)  where {ArgT}
  ...
  @assign outExp = begin
    ...
    ## REENABLE @debug "Calling mapFoldShallow"
    @match exp begin
...
function toDAE(exp::Expression)::DAE.Exp
  ...
  @assign dexp = begin
    ...
    @match exp begin
      ...
      CREF_EXPRESSION(__)  => begin
        DAE.COMPONENT_REF_CREF(toDAE(exp.cref), toDAE(exp.ty))
      end
  ...
  @assign var = begin
    @match binding begin
      ...
      _ => begin
        ...
        ## REENABLE @debug "Got Unknown binding!"
        fail()
      end
    end
  end
