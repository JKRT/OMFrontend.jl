    ...
    @match outTree begin
      ...
      NODE(__) => begin
        ...
        elseif outTree.height != max(lh, rh) + 1
          #= Complex assign=#@assign outTree.height = max(lh, rh) + 1
   ...
   tree = begin
    ...
    @match tree begin
      ...
      NODE(key = key) => begin
         key_comp = keyCompare(inKey, key)
        if key_comp == (-1)
          #= complex assign=#@assign tree.left = add(tree.left, inKey, inValue, conflictFunc)
        elseif key_comp == 1
          #= complex assign=#@assign tree.right = add(tree.right, inKey, inValue, conflictFunc)
        else
           value = conflictFunc(inValue, tree.value, key)
          if !referenceEq(tree.value, value)
            #= complex assign=#@assign tree.value = value
          end
        end
        #=  Replace left branch.
        =#
    ...
    @match outTree begin
      NODE(key = key, value = value) => begin
        ...
        if !referenceEq(new_branch, outTree.left)
          #= Complex assign=#@assign outTree.left = new_branch
        end
         new_value = inFunc(key, value)
        if !referenceEq(value, new_value)
          #= Complex assign=#@assign outTree.value = new_value
        end
         new_branch = map(outTree.right, inFunc)
        if !referenceEq(new_branch, outTree.right)
          #= Complex assign=#@assign outTree.right = new_branch
        end
        outTree
      end
      LEAF(key = key, value = value) => begin
