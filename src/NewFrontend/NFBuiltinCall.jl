  ...
  @assign () = begin
    @match call begin
      ...
      _  => begin
        ...
        # ## REENABLE @debug "Unknown call: $call"
        fail()
      end
    end
  end
...
function typeSpecial(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, NFType, VariabilityType}
...
function makeCatExp(n::Integer, args::List{<:Expression}, tys::List{<:M_Type}, variability::VariabilityType, info::SourceInfo) ::Tuple{Expression, M_Type}
  ...
  for arg in args2
    @match _cons(ty, tys2) = tys2
    @assign pos = pos - 1
    @assign (arg2, ty1, mk) = TypeCheck.matchTypes(ty, resTyToMatch, arg, allowUnknown = true)
    if TypeCheck.isIncompatibleMatch(mk)
...
function typeOverloadedStringCall(overloadedType::M_Type, args::List{<:TypedArg}, namedArgs::List{<:TypedNamedArg}, call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  try
    @assign fn_ref = P_Function.lookupFunctionSimple("'String'", recopnode)
  catch
  ...
  =#
  @assign fn_ref = P_Function.instFunctionRef(fn_ref, info(recopnode))
  @assign candidates = P_Function.typeRefCache(fn_ref)
  ...
  #= end for;
  =#
  @assign matchedFunctions = P_Function.matchFunctionsSilent(candidates, args, namedArgs, info)
...
function typeDerCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, NFType, VariabilityType}
  ...
  # ## REENABLE @info "Value of origin was $origin, $ORIGIN_FUNCTION"
  if flagSet(origin, ORIGIN_FUNCTION)
    #Error.addSourceMessage(Error.EXP_INVALID_IN_FUNCTION, list("der"), info)
    # ## REENABLE @debug "Der used in context: $origin"
    fail()
  end
  @match UNTYPED_CALL(ref = fn_ref, arguments = args, named_args = named_args) = call
...
function typeEdgeCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  @match (@match ARG_TYPED_CALL(CREF(node = fn_node), args, _) = argtycall) = P_Call.typeNormalCall(call, origin, info)
...
function typeFillCall2(fnRef::ComponentRef, fillType::M_Type, fillArg::Expression, dimensionArgs::List{<:Expression}, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  @match list(fn) = P_Function.typeRefCache(fnRef)
  @assign ty = Type.liftArrayLeftList(fillType, dims)
  if evaluated
...
function typeCatCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  @assign (arg, ty, variability) = typeExp(arg, origin, info)
  @assign (arg, ty, mk) = TypeCheck.matchTypes(ty, TYPE_INTEGER(), arg)
...
function typeSymmetricCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, NFType, VariabilityType}
  local variability::VariabilityType
  local ty::NFType
  local callExp::Expression
  local fn_ref::ComponentRef
...
function typeTransposeCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, NFType, VariabilityType}
  local variability::VariabilityType
  local ty::NFType
  local callExp::Expression
...
function typeNoEventCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  @match list(arg) = args
  @assign (arg, ty, variability) = typeExp(arg, ExpOrigin.setFlag(origin, ExpOrigin.NOEVENT), info)
  @match list(fn) = P_Function.typeRefCache(fn_ref)
  @assign callExp = CALL_EXPRESSION(P_Call.makeTypedCall(fn, list(arg), variability, ty))
  (callExp, ty, variability)
end
...
function typeClockCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  @match TYPED_CALL(arguments = args) = P_Call.typeMatchNormalCall(call, origin, info)
...
function typeSampleCall(call::Call, origin::ORIGIN_Type, info::SourceInfo) ::Tuple{Expression, M_Type, Variability}
  ...
  @assign fn_ref = P_Function.instFunctionRef(fn_ref, info(recopnode))
  @match list(normalSample, clockedSample) = P_Function.typeRefCache(fn_ref)
