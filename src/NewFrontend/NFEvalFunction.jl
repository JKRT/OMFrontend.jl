...
function assignVariable(variable::Expression, value::Expression)
  return @assign () = begin
    ...
    local var_ptr::Mutable{Expression}
    #=  variable := value
    =#
    @match (variable, value) begin
      (P_Expression.Expression.MUTABLE(exp = var_ptr), _) => begin
      ...
      (
        P_Expression.Expression.SUBSCRIPTED_EXP(
          exp = P_Expression.Expression.MUTABLE(exp = var_ptr),
        ),
        _,
      ) => begin
...
function assignSubscriptedVariable(
  ...
  return Mutable.update(variable, assignArrayElement(Mutable.access(variable), subs, value))
end
...
function evaluateFor(
  ...
  local iter_exp::Mutable{Expression}
  ...
  if P_RangeIterator.RangeIterator.hasNext(range_iter)
    @match EXP_NODE(exp = P_Expression.Expression.MUTABLE(exp = iter_exp)) =
      iterator
...
function evaluateAssert(condition::Expression, assertStmt::Statement)::FlowControl
  ...
  if P_Expression.Expression.isFalse(Ceval.evalExp(condition, target))
    ...
    @assign () = begin
      @match (msg, lvl) begin
        (
          P_Expression.Expression.STRING(__),
...
function evaluateKnownExternal(name::String, args::List{<:Expression})::Expression
  ...
  @assign result = begin
    ...
    @match (name, args) begin
      ("ModelicaInternal_countLines", P_Expression.Expression.STRING(s1) <| nil()) =>
        begin
          P_Expression.Expression.INTEGER(ModelicaExternalC.Streams_countLines(s1))
        end

      ("ModelicaInternal_fullPathName", P_Expression.Expression.STRING(s1) <| nil()) =>
        begin
          P_Expression.Expression.STRING(ModelicaExternalC.File_fullPathName(s1))
        end

      (
        "ModelicaInternal_print",
        P_Expression.Expression.STRING(s1) <| P_Expression.Expression.STRING(s2) <| nil(),
      ) => begin
      ...
      (
        ...
        P_Expression.Expression.STRING(s1) <| P_Expression.Expression.INTEGER(i) <| nil(),
      ) => begin
        @assign (s1, b) = ModelicaExternalC.Streams_readLine(s1, i)
        TUPLE_EXPRESSION(
          TYPE_TUPLE(list(TYPE_STRING(), TYPE_BOOLEAN()), NONE()),
          list(P_Expression.Expression.STRING(s1), P_Expression.Expression.BOOLEAN(b)),
        )
      end

      ("ModelicaInternal_stat", P_Expression.Expression.STRING(s1) <| nil()) => begin
        @assign i = ModelicaExternalC.File_stat(s1)
        listGet(FILE_TYPE_LITERALS, i)
      end

      ("ModelicaStreams_closeFile", P_Expression.Expression.STRING(s1) <| nil()) => begin
      ...
      (
        ...
        P_Expression.Expression.STRING(s1) <|
        P_Expression.Expression.STRING(s2) <| P_Expression.Expression.BOOLEAN(b) <| nil(),
      ) => begin
        @assign i = ModelicaExternalC.Strings_compare(s1, s2, b)
        listGet(COMPARE_LITERALS, i)
      end

      ("ModelicaStrings_length", P_Expression.Expression.STRING(s1) <| nil()) => begin
        P_Expression.Expression.INTEGER(stringLength(s1))
      end

      (
        "ModelicaStrings_scanReal",
        P_Expression.Expression.STRING(s1) <|
        P_Expression.Expression.INTEGER(i) <| P_Expression.Expression.BOOLEAN(b) <| nil(),
      ) => begin
      ...
      (
        ...
        TUPLE_EXPRESSION(
          ...
          list(P_Expression.Expression.INTEGER(i), P_Expression.Expression.INTEGER(i2)),
        )
      end
      ...
      (
        ...
        TUPLE_EXPRESSION(
          ...
          list(P_Expression.Expression.INTEGER(i), P_Expression.Expression.STRING(s2)),
        )
      end
      ...
      (
        ...
        P_Expression.Expression.INTEGER(ModelicaExternalC.Strings_skipWhiteSpace(s1, i))
      end

      (
        "ModelicaStrings_substring",
        P_Expression.Expression.STRING(s1) <|
        P_Expression.Expression.INTEGER(i) <| P_Expression.Expression.INTEGER(i2) <| nil(),
      ) => begin
        P_Expression.Expression.STRING(System.substring(s1, i, i2))
      end

      ("ModelicaStrings_hashString", P_Expression.Expression.STRING(s1) <| nil()) => begin
        P_Expression.Expression.INTEGER(ModelicaExternalC.Strings_hashString(s1))
      end
      ...
      (
        ...
        P_Expression.Expression.ARRAY(
          ...
          list(
            P_Expression.Expression.INTEGER(dims[1]),
            P_Expression.Expression.INTEGER(dims[2]),
          ),
          true,
        )
      end
      ...
      (
        ...
        P_Expression.Expression.STRING(s1) <|
        P_Expression.Expression.STRING(s2) <|
        P_Expression.Expression.INTEGER(i) <|
        P_Expression.Expression.INTEGER(i2) <| P_Expression.Expression.BOOLEAN(b) <| nil(),
      ) => begin
...
function evaluateOpenModelicaRegex(args::List{<:Expression})::Expression
  ...
  @assign result = begin
    @match args begin
      ...
      P_Expression.Expression.STRING(re) <|
      P_Expression.Expression.INTEGER(i) <|
      P_Expression.Expression.BOOLEAN(extended) <|
      P_Expression.Expression.BOOLEAN(insensitive) <| nil() => begin
        @assign (n, strs) = System.regex(str, re, i, extended, insensitive)
        @assign expl = List(P_Expression.Expression.STRING(s) for s in strs)
        @assign strs_ty =
        ...
        TUPLE_EXPRESSION(
          ...
          list(P_Expression.Expression.INTEGER(n), strs_exp),
        )
      end
