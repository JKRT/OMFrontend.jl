...
function expandCast(exp::Expression, ty::M_Type)::Tuple{Expression, Bool}
  ...
  if expanded
    @assign outExp = P_Expression.Expression.typeCast(outExp, ty)
  else
    @assign outExp = exp
  end
  return (outExp, expanded)
...
function makeLogicalUnaryOp(exp1::Expression, op::Operator)::Expression
  local exp::Expression = P_Expression.Expression.LUNARY(op, exp1)
  return exp
end
...
function expandLogicalUnary(exp::Expression, op::Operator)::Tuple{Expression, Bool}
  ...
  if expanded
    @assign outExp = P_Expression.Expression.mapArrayElements(
      outExp,
...
function makeLBinaryOp(exp1::Expression, op::Operator, exp2::Expression)::Expression
  ...
  if P_Expression.Expression.isScalarLiteral(exp1) &&
    ...
    @assign exp = P_Expression.Expression.LBINARY(exp1, op, exp2)
  end
  return exp
end
...
function expandLogicalBinary(exp::Expression)::Tuple{Expression, Bool}
  ...
  @match P_Expression.Expression.LBINARY(exp1 = exp1, operator = op, exp2 = exp2) = exp
  if Type.isArray(P_Operator.Operator.typeOf(op))
  ...
  @assign (outExp, expanded) = begin
    @match exp2 begin
      P_Expression.Expression.INTEGER(0) => begin
        ...
        =#
        @assign n =
          P_Dimension.Dimension.size(listHead(Type.arrayDims(P_Operator.Operator.typeOf(
            op,
          ))))
...
function makeBinaryMatrixProduct(exp1::Expression, exp2::Expression)::Expression
  ...
  @match P_Expression.Expression.ARRAY(Type.ARRAY(ty, list(n, _)), expl1) = exp1
  ...
  =#
  @match P_Expression.Expression.ARRAY(Type.ARRAY(dimensions = list(p, _)), expl2) =
    P_Expression.Expression.transposeArray(exp2)
  @assign mat_ty = Type.ARRAY(ty, list(n, p))
  if listEmpty(expl2)
    @assign exp = P_Expression.Expression.makeZero(mat_ty)
  else
    @assign row_ty = Type.ARRAY(ty, list(p))
    @assign expl1 = List(
...
function makeScalarProduct(exp1::Expression, exp2::Expression)::Expression
  ...
  @match P_Expression.Expression.ARRAY(ty, expl1) = exp1
  @match P_Expression.Expression.ARRAY(_, expl2) = exp2
...
function expandBinaryMatrixVector(exp::Expression)::Tuple{Expression, Bool}
  ...
  @assign (exp1, expanded) = expand(exp1)
  if expanded
    @match P_Expression.Expression.ARRAY(Type.ARRAY(ty, list(n, _)), expl) = exp1
    @assign ty = Type.ARRAY(ty, list(n))
    if listEmpty(expl)
...
function expandBinaryVectorMatrix(exp::Expression)::Tuple{Expression, Bool}
  ...
  @assign (exp2, expanded) = expand(exp2)
  if expanded
    @match P_Expression.Expression.ARRAY(Type.ARRAY(ty, list(m, _)), expl) =
      P_Expression.Expression.transposeArray(exp2)
    @assign ty = Type.ARRAY(ty, list(m))
    if listEmpty(expl)
      @assign outExp = P_Expression.Expression.makeZero(ty)
    else
      @assign (exp1, expanded) = expand(exp1)
...
function expandBinaryArrayScalar(
  ...
  @assign (exp1, expanded) = expand(exp1)
  if expanded
    @assign op = P_Operator.Operator.OPERATOR(
      Type.arrayElementType(P_Operator.Operator.typeOf(op)),
      scalarOp,
    )
    @assign outExp = P_Expression.Expression.mapArrayElements(
      exp1,
...
function expandBinaryElementWise2(
  ...
  @assign eop = P_Operator.Operator.setType(Type.unliftArray(ty), op)
...
function expandSize(exp::Expression)::Tuple{Expression, Bool}
  ...
  @assign outExp = begin
    ...
    local expl::List{Expression}
    @match exp begin
      P_Expression.Expression.SIZE(exp = e, dimIndex = NONE()) => begin
        ...
        P_Expression.Expression.makeArray(
          Type.ARRAY(ty, list(P_Dimension.Dimension.fromInteger(dims))),
          expl,
        )
      end
...
function expandBuiltinGeneric2(
  ...
  @assign exp = begin
    local expl::List{Expression}
    @match exp begin
      P_Expression.Expression.ARRAY(literal = true) => begin
        exp
      end
      ...
      P_Expression.Expression.ARRAY(__) => begin
        ...
        P_Expression.Expression.makeArray(Type.setArrayElementType(exp.ty, ty), expl)
      end
...
function expandRange(exp::Expression)::Tuple{Expression, Bool}
  ...
  @match P_Expression.Expression.RANGE(ty = ty) = exp
  @assign expanded = Type.hasKnownSize(ty)
  if expanded
...
function expandTypename(ty::M_Type)::Expression
  ...
  @assign outExp = begin
    ...
    @match ty begin
      Type.ARRAY(elementType = TYPE_BOOLEAN(__)) => begin
        P_Expression.Expression.makeArray(
          ty,
          list(
      ...
      Type.ARRAY(elementType = TYPE_ENUMERATION(__)) => begin
...
function expand(exp::Expression)::Tuple{Expression, Bool}
  ...
  @assign (exp, expanded) = begin
    ...
    @match exp begin
      ...
      P_Expression.Expression.ARRAY(ty = Type.ARRAY(dimensions = nil())) => begin
        (exp, true)
      end
