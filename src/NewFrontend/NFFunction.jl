...
function getLocalArguments(fn::M_Function)::List{Expression}
  ...
  for l in fn.locals
    if isComponent(l)
      binding = getBinding(component(l))
      # Error.assertion(
      #   hasExp(binding),
      #   getInstanceName() + " got local component without binding",
      #   sourceInfo(),
      # )
      localArgs = _cons(getExp(binding), localArgs)
    end
  end
...
function foldExpParameter(node::InstNode, foldFn::FoldFunc, arg::ArgT) where {ArgT}
  ...
  comp = component(node)
  arg = foldExp(getBinding(comp), foldFn, arg)
  () = begin
    @match comp begin
...
function foldExp(
  ...
  if mapBody
    arg = P_Sections.Sections.foldExp(getSections(cls), foldFn, arg)
  end
  return arg
end
...
function mapExpParameter(node::InstNode, mapFn::MapFunc)
  ...
  if !referenceEq(binding, binding2)
    comp = P_Component.setBinding(binding2, comp)
    dirty = true
  end
  () = begin
    @match comp begin
      TYPED_COMPONENTy(__) => begin
        ...
        if !referenceEq(ty, comp.ty)
          #= complex assign=#@assign comp.ty = ty
          dirty = true
        end
        cls = getClass(comp.classInst)
        applyComponents(
...
function mapExp(
  ...
  if mapParameters
    ...
    #= Complex assign=#@assign fn.returnType = makeReturnType(fn)
  end
  if mapBody
    sections = P_Sections.Sections.mapExp(getSections(cls), mapFn)
...
function makeDAEType(fn::M_Function, boxTypes::Bool = false)::DAE.Type
  ...
  for param in fn.inputs
    comp = component(param)
    pname = name(param)
    ty = getType(comp)
    ptype = toDAE(if boxTypes
      Type.box(ty)
    else
      ty
    end)
    pconst = P_Prefixes.variabilityToDAEConst(variability(comp))
    ppar = P_Prefixes.parallelismToDAE(P_Component.parallelism(comp))
  ...
  outType = DAE.T_FUNCTION(params, toDAE(ty), fn.attributes, fn.path)
  return outType
end
...
function setFunctionPointer(isPointer::Bool, fn::M_Function)::M_Function
  ...
  #= Complex assign=#@assign attr.isFunctionPointer = isPointer
  #= complex assign=#@assign fn.attributes = attr
  return fn
end
...
function applyPartialApplicationArg(
  ...
  while !listEmpty(rest_inputs)
    ...
    outInputs = _cons(i, outInputs)
    outSlots = _cons(s, outSlots)
  end
  Error.addSourceMessage(
...
function typePartialApplication(
  ...
  variability = if isImpure(fn) || isOMImpure(fn)
  ...
  for arg in args
    ...
    ty_args = _cons(box(arg), ty_args)
    variability = variabilityMax(variability, arg_var)
  end
  #= complex assign=#@assign fn.inputs = inputs
  #= complex assign=#@assign fn.slots = slots
  ty = TYPE_FUNCTION(fn, FunctionTYPE_FUNCTIONAL_VARIABLE)
  exp = PARTIAL_FUNCTION_APPLICATION_EXPRESSION(
    fn_ref,
...
function boxFunctionParameter(component::InstNode)
  ...
  comp = P_Component.setType(Type.box(getType(comp)), comp)
...
function typeFunctionSignature(fn::M_Function)::M_Function
  ...
  if !isTyped(fn)
    ...
    #= complex assign=#@assign fn.slots = makeSlots(fn.inputs)
    checkParamTypes(fn)
    #= Complex assign=#@assign fn.returnType = makeReturnType(fn)
  end
  return fn
...
function matchArgVectorized(
  ...
  =#
  rest_ty = liftArrayLeftList(arrayElementType(argTy), rest_dims)
...
function lookupSlotInArray(slotName::String, slots::Array{<:Slot})::Option{Slot}
  ...
  try
    slot = ArrayUtil.getMemberOnTrue(slotName, slots, P_Slot.hasName)
    outSlot = SOME(slot)
  catch
    outSlot = NONE()
  end
  ...
  outArg = begin
    ...
    @match slot.evalStatus begin
      ...
      SlotEvalStatus.NOT_EVALUATED => begin
        ...
        =#
        #= Complex assign=#@assign slot.evalStatus = SlotEvalStatus.EVALUATING
        ...
        #= complex assign=#@assign slot.arg = SOME(outArg)
        #= Complex assign=#@assign slot.evalStatus = SlotEvalStatus.EVALUATED
        arrayUpdate(slots, slot.index, slot)
        outArg
      end
    end
  ...
  for i = arrayLength(slots):(-1):1
    ...
    if s.name == argName
      if !P_Slot.named(s)
        matching = false
      elseif isNone(s.arg)
        #= complex assign=#@assign s.arg = SOME((argExp, ty, var))
        slots[i] = s
      else
...
function fillArgs(
  ...
  for arg in args
    ...
    #= complex assign=#@assign slot.arg = SOME(arg)
...
function setReturnType(ty::M_Type, fn::M_Function)::M_Function
  ...
  #= Complex assign=#@assign fn.returnType = ty
  return fn
end
...
function toFlatStream(fn::M_Function, s)
  ...
  if isDefaultRecordConstructor(fn)
    s = IOStream.append(s, toFlatString(fn.node))
  else
    ...
    for i in fn.inputs
      s = IOStream.append(s, "  ")
      s = IOStream.append(s, toFlatString(i))
      s = IOStream.append(s, ";\\n")
    end
    for o in fn.outputs
      s = IOStream.append(s, "  ")
      s = IOStream.append(s, toFlatString(o))
      s = IOStream.append(s, ";\\n")
    end
    if !listEmpty(fn.locals)
      ...
      for l in fn.locals
        s = IOStream.append(s, "  ")
        s = IOStream.append(s, toFlatString(l))
        s = IOStream.append(s, ";\\n")
      end
    end
    fn_body = getBody(fn)
    if !listEmpty(fn_body)
      ...
      s = P_Statement.Statement.toFlatStreamList(fn_body, "  ", s)
    end
...
function signatureString(fn::M_Function, printTypes::Bool = true)::String
  ...
  for s in fn.slots
    input_str = ""
    c = component(listHead(inputs))
    inputs = listRest(inputs)
    if isSome(s.default)
      @match SOME(def_exp) = s.default
      input_str = " = " + toString(def_exp)
    end
    ...
    if printTypes && P_Component.isTyped(c)
      ty = getType(c)
      var_s = P_Prefixes.unparseVariability(variability(c), ty)
...
function setName(name::Absyn.Path, fn::M_Function)::M_Function
  ...
  #= complex assign=#@assign fn.path = name
  return fn
end
...
function mapCachedFuncs(inNode::InstNode, mapFn::MapFn)
  ...
  cache = begin
    @match cache begin
      C_FUNCTION(__) => begin
        #= complex assign=#@assign cache.funcs = list(mapFn(fn) for fn in cache.funcs)
        cache
      end
  ...
  (fnNode, specialBuiltin) = begin
    ...
    @match def begin
      SCode.CLASS(__) where {(SCodeUtil.isOperatorRecord(def))} => begin
        ...
        fnNode = instConstructor(fnPath, fnNode, info)
        (fnNode, false)
      end
      ...
      SCode.CLASS(__) => begin
        ...
        #= complex assign=#@assign fn.derivatives =
  ...
  attr = begin
    ...
    @matchcontinue fres begin
      SCode.FR_EXTERNAL_FUNCTION(is_impure) => begin
        in_params = list(name(i) for i in inputs)
        out_params = list(name(o) for o in outputs)
        nameVar = SCodeUtil.isBuiltinFunction(def, in_params, out_params)
        inline_ty = commentIsInlineFunc(cmt)
        is_impure = is_impure || hasImpure(cmt)
      ...
      _ => begin
        ...
        #=  Normal function.
        =#
        #        inline_ty = commentIsInlineFunc(cmt) TODO
...
function paramDirection(@nospecialize(componentArg::InstNode))::DirectionType
  ...
  var = variability(component(componentArg))
            ...
            () = begin
              @match paramDirection(n) begin
                Direction.INPUT => begin
                  ...
                  =#
                  inputs = _cons(n, inputs)
                  ()
                end

                Direction.OUTPUT => begin
                  outputs = _cons(n, outputs)
                  ()
                end

                Direction.NONE => begin
                  locals = _cons(n, locals)
                  ()
                end
              end
            end
  ...
  () = begin
    @match exp begin
      CREF_EXPRESSION(__) => begin
        params = ListUtil.deleteMemberOnTrue(
          exp.cref,
          params,
          containsNode,
        )
...
function analyseUnusedParametersExp(
  ...
  if !listEmpty(params)
    params = fold(exp, analyseUnusedParametersExp2, params)
  end
  return params
end
...
function analyseUnusedParameters(fn::M_Function)::List{Int}
  ...
  for i in inputs
    ...
    unusedInputs = _cons(index, unusedInputs)
  end
  return unusedInputs
end
