...
function lookupBaseClassName(name::Absyn.Path, scope::InstNode, info::SourceInfo) ::List{InstNode}
  ...
  P_LookupState.assertClass(state, listHead(nodes), name, info)
  nodes
end

function lookupComponent(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#, info::SourceInfo) ::Tuple{ComponentRef, InstNode}
  ...
  try
    ...
    @match false = isName(node)
  catch
    Error.addSourceMessageAndFail(Error.LOOKUP_VARIABLE_ERROR, list(Dump.printComponentRefStr(cref), scopeName(scope)), info)
  end
  @assign state = fixTypenameState(node, state)
  P_LookupState.assertComponent(state, node, cref, info)
  (foundCref, foundScope #= The scope the cref was found in. =#)
end

function lookupConnector(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#, info::SourceInfo) ::Tuple{ComponentRef, InstNode}
  local foundScope::InstNode #= The scope the cref was found in. =#
  local foundCref::ComponentRef
...
function fixTypenameState(component::InstNode, state::LookupState) ::LookupState


  local ty::M_Type

  if isClass(component)
    @assign ty = getType(Inst.expand(component))
    @assign state = begin
      @match ty begin
...
""" #= Looks up a component in the local scope, without searching in any enclosing
                 ...
                 scope in the case where the cref refers to an outer component. =#"""
                   function lookupLocalComponent(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#, info::SourceInfo) ::Tuple{ComponentRef, InstNode}
                     local foundScope::InstNode #= The scope the cref was found in. =#
                     local foundCref::ComponentRef
                     local state::LookupState
                     local node::InstNode
                     @assign (foundCref, foundScope, state) = lookupLocalCref(cref, scope, info)
                     P_LookupState.assertComponent(state, node(foundCref), cref, info)
                     (foundCref, foundScope #= The scope the cref was found in. =#)
                   end

function lookupFunctionName(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#, info::SourceInfo) ::Tuple{ComponentRef, InstNode}
  ...
  try
    ...
    Error.addSourceMessageAndFail(Error.LOOKUP_FUNCTION_ERROR, list(Dump.printComponentRefStr(cref), scopeName(scope)), info)
  end
  @assign (foundCref, state) = fixExternalObjectCall(node, foundCref, state)
  P_LookupState.assertFunction(state, node, cref, info)
  (foundCref, foundScope)
end

function lookupFunctionNameSilent(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#) ::Tuple{ComponentRef, InstNode}
  local foundScope::InstNode
  local foundCref::ComponentRef
  ...
  @assign node = node(foundCref)
  @assign (foundCref, state) = fixExternalObjectCall(node, foundCref, state)
  @match true = P_LookupState.isFunction(state, node)
  (foundCref, foundScope)
end
                 ...
                 is changed to
                   'ExtObj eo = ExtObj.constructor(...)' =#"""
                     function fixExternalObjectCall(node::InstNode, cref::ComponentRef, state::LookupState) ::Tuple{ComponentRef, LookupState}
                       ...
                       =#
                       if ! P_LookupState.isClass(state)
                         return (cref, state)
                       end
                       Inst.expand(node)
...
function lookupImport(name::Absyn.Path, scope::InstNode, info::SourceInfo) ::InstNode
  ...
  P_LookupState.assertImport(state, element, name, info)
  element
end

function lookupCrefWithError(cref::Absyn.Path, scope::InstNode, info::SourceInfo, errMsg) ::Tuple{ComponentRef, InstNode, LookupState}
  local state::LookupState
  local foundScope::InstNode
  local foundCref::ComponentRef
  try
...
""" #= This function will look up an Absyn.ComponentRef in the given scope, and
                 ...
                 of the cref was found will also be returned. =#"""
                   function lookupCref(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#) ::Tuple{ComponentRef, InstNode, LookupState}
                     local state::LookupState
...
function lookupLocalCref(cref::Absyn.Path, scope::InstNode #= The scope to look in. =#, info::SourceInfo) ::Tuple{ComponentRef, InstNode, LookupState}
  local state::LookupState
  local foundScope::InstNode #= The scope where the first part of the cref was found. =#
  local foundCref::ComponentRef
  ...
  @assign (foundCref, foundScope, state) = begin
    ...
    @matchcontinue cref begin
      Absyn.CREF_IDENT(__)  => begin
        ...
        @assign state = P_LookupState.nodeState(node)
...
function lookupLocalSimpleName(n::String, scope::InstNode) ::Tuple{InstNode, Bool}
  ...
  local node::InstNode
  @info "Looking up simple name $n in $scope"
  @assign (node, isImport) = lookupElement(n, getClass(scope))
  @info "Our node is $node"
  @assign node = resolveInner(node)
  strace = stacktrace()
  @info "After resolveInner our stacktrace is: $strace"
                       ...
                       @match name begin
                         ...
                         Absyn.QUALIFIED(__)  => begin
                           ...
                           if is_import
                             ...
                             @assign state = P_LookupState.next(node, state, checkAccessViolations)
                     ...
                     @assign (nodes, state) = begin
                       @match name begin
                         Absyn.IDENT(__)  => begin
                           ...
                           @info "Here we are!"
                           @assign state = P_LookupState.next(node, state)
...
function lookupCrefInNode(cref::Absyn.Path, node::InstNode, foundCref::ComponentRef, foundScope::InstNode, state::LookupState) ::Tuple{ComponentRef, InstNode, LookupState}
  ...
  local cls::Class
  local is_import::Bool
  if P_LookupState.isError(state)
  ...
  @assign state = P_LookupState.next(n, state)
  @assign (foundCref, foundScope, state) = begin
    @match cref begin
      Absyn.CREF_IDENT(__)  => begin
