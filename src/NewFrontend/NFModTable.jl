...
"""
modtable specific add. Calls merge instead of conflict func.
"""
function add(
  inTree::Tree,
  inKey::Key,
   ...
   tree = begin
    ...
    @match tree begin
      ...
      NODE(key = key) => begin
         key_comp = keyCompare(inKey, key)
        if key_comp == (-1)
           tree.left = add(tree.left, inKey, inValue, scope, prefix)
        elseif key_comp == 1
           tree.right = add(tree.right, inKey, inValue, scope, prefix)
        else
           value = conflictFunc(inValue, tree.value, key)
          if !referenceEq(tree.value, value)
             tree.value = value
...
function fromVector(
  ...
  for i in 1:length(inValues)
    tree = add(tree, inKeys[i], inValues[i], scope, prefix)
  end
  return tree
end
...
function join(tree::Tree, treeToJoin::Tree, scope, prefix)
   tree = begin
    @match treeToJoin begin
      EMPTY(__) => begin
        tree
      ...
      NODE(__) => begin
         tree = add(tree, treeToJoin.key, treeToJoin.value, scope, prefix)
      ...
      LEAF(__) => begin
        add(tree, treeToJoin.key, treeToJoin.value, scope, prefix)
      end
    end
  end
  return tree
