...
function instDefaultConstructor(
  ...
  try
     ctor_node = lookupLocalSimpleName(
      name(node),
...
function collectRecordParams(
  ...
  local tree::ClassTree
   tree = classTree(getClass(recNode))
   () = begin
    @match tree begin
...
function collectRecordFields(recNode::InstNode)::List{Field}
  ...
  local tree::ClassTree
   tree = classTree(getClass(recNode))
   fields = foldComponents(tree, collectRecordField, nil)
...
function collectRecordField(component::InstNode, fields::List{<:Field})::List{Field}
  ...
  local comp::Component
  if isProtected(comp_node)
     fields = _cons(P_Field.LOCAL(name(comp_node)), fields)
  else
     comp = component(comp_node)
    if P_Component.isConst(comp) && P_Component.hasBinding(comp)
       fields = _cons(P_Field.LOCAL(name(comp_node)), fields)
    elseif !P_Component.isOutput(comp)
       fields = _cons(P_Field.INPUT(name(comp_node)), fields)
    end
  end
  return fields
end
...
function fieldsToDAE(fields::List{<:Field})::List{String}
  ...
  for field in fields
     () = begin
      @match field begin
        INPUT(__) => begin
           fieldNames = _cons(field.name, fieldNames)
          ()
        end
        _ => begin
          ()
