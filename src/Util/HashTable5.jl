...
function hashFunc(cr::Key, mod::Integer)::Integer
  ...
  @assign crstr = Dump.printComponentRefStr(cr)
  @assign res = stringHashDjb2Mod(crstr, mod)
  return res
end
...
function emptyHashTableSized(size::Integer)::HashTable
  ...
  @assign hashTable = BaseHashTable.emptyHashTableWork(
    size,
    (hashFunc, AbsynUtil.crefEqual, Dump.printComponentRefStr, intString),
  )
  return hashTable
end
