...
function emptyHashTableSized(size::Integer)::HashTable
  ...
  @assign hashTable = BaseHashTable.emptyHashTableWork(
    ...
    (
      ...
      printIntListArrayStr,
    ),
  )
  return hashTable
end

function printIntListArrayStr(iValue::Tuple{<:List{<:Integer}, Array{<:Integer}})::String
  local res::String

  local iList::List{Integer}
  local iArray::Array{Integer}

  @assign (iList, iArray) = iValue
  @assign res =
    "[" +
    stringDelimitList(ListUtil.map(iList, intString), ",") +
    "] {" +
    stringDelimitList(ListUtil.map(arrayList(iArray), intString), ",") +
    "}"
  return res
end
