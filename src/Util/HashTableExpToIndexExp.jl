...
function emptyHashTableSized(size::Integer)::HashTable
  ...
  @assign hashTable = BaseHashTable.emptyHashTableWork(
    size,
    (Expression.hashExpMod, Expression.expEqual, ExpressionDump.printExpStr, intExpListStr),
  )
  return hashTable
end

function intExpListStr(intExplst::List{<:Tuple{<:Integer, DAE.Exp}})::String
  local res::String
  ...
  for elem in intExplst
    @assign combStr = _cons(
      intString(Util.tuple21(elem)) + ": " + ExpressionDump.printExpStr(Util.tuple22(elem)),
      combStr,
    )
  end
  @assign res = "{" + stringDelimitList(combStr, ",") + "}"
  return res
end
