module HashTablePathToFunction
...
function hashFunc(path::Key, mod::Integer)::Integer
  local res::Integer

  local str::String

  @assign str = AbsynUtil.pathString(path)
  @assign res = stringHashDjb2Mod(str, mod)
  return res
end

function valString(v::Value)::String
  local str::String

  @assign str = "/* a function of some kind */"
  return str
end
...
function emptyHashTableSized(size::Integer)::HashTable
  ...
  @assign hashTable = BaseHashTable.emptyHashTableWork(
    size,
    (hashFunc, AbsynUtil.pathEqual, AbsynUtil.pathString, valString),
  )
  return hashTable
end
