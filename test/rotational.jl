@testset "Rotational" begin
  @testset "Components" begin
    ...
    # @test typeof(flattenModelInMSL("$(prefix).ElastoBacklash2")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).BearingFriction")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).Brake")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).Clutch")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).OneWayClutch")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).IdealGear")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).LossyGear")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).IdealPlanetary")[1]) == OMFrontend.Main.FLAT_MODEL
    # @test typeof(flattenModelInMSL("$(prefix).Gearbox")[1]) == OMFrontend.Main.FLAT_MODEL
